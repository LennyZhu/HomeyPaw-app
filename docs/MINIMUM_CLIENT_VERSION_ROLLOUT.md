...
R3 does not version-gate every Journal/Care/Chat/Schedule write after release. During migration, the independent pre-cutover lock blocks all application-table writes, including those domains, even when the caller uses an old binary without version headers. The lock is seeded ON by `20260914110000_pre_cutover_release_lock.sql`, before Journal Video foundation and Phase A. The R3 migration neither updates nor unlocks it. Read requests remain available; sign-up may fail closed because it writes a profile. Authentication sign-in remains available. A review must confirm any additional old-client incompatibility before unlocking.

## Production sequence — template only; no Production action in R5A

1. Confirm Production migration history, the recorded R4 preflight, backup, and the actual deployed Storage/Edge versions. Do not proceed if the first pending migration is not `20260914110000_pre_cutover_release_lock.sql` or if the production Storage service rejects its restrictive policies/trigger.
...
## Bootstrap order regression

The bootstrap only guards legacy tables that already exist, plus Storage and Auth deletion. Journal Video foundation attaches the same guard to `post_videos` and `media_cleanup_jobs` immediately after creating each table; Phase A attaches it to all three Family tables. `post_videos` has no authenticated DML grants: authenticated mutation goes through `create_post_v2` / `update_post_v2`, whose posts and video writes are guarded. Thumbnail metadata lives in `post_videos` and `storage.objects`, not a separate thumbnail table. Queue DML is service-role-only and is also guarded. Migration sessions remain trusted direct `postgres` sessions.

Run `node scripts/verify-pre-cutover-migration-order.mjs` against local Docker. It resets **only the local database**, tests interruption after bootstrap / Video / Phase A / R3, and restores the complete migration chain. Do not run domain fixture verifiers until the ordinary lock verifier has explicitly unlocked the local database. The renamed initializer has never been applied to Production; an existing disposable test project with the old timestamp needs a separately reviewed synchronization plan, not an automatic history repair.
